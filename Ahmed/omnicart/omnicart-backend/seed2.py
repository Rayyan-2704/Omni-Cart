"""
OmniCart Seed Script v2 (idempotent)
------------------------------------
Uses Flask-Bcrypt for password hashing.
Safe to run multiple times — skips existing records.

Run from omnicart-backend folder with venv active:
    python seed2.py
"""
...
def seed():
    with app.app_context():
        ...
        # ── 1. Admins ─────────────────────────────────────────────────────────
        admins_raw = [
            ("Super Admin",   "admin@omnicart.com",         "Admin@1234"),
            ("Ops Admin",     "ops.admin@omnicart.com",     "Admin@1234"),
            ("Support Admin", "support.admin@omnicart.com", "Admin@1234"),
        ]
        ac = 0
        for name, email, pw in admins_raw:
            _, created = get_or_create(
                ...
                defaults={"name": name, "password_hash": hp(pw)},
            )
            if created: ac += 1
        ...
        for name, email, phone, address in customers_raw:
            c, created = get_or_create(
                ...
                defaults={
                    "name":          name,
                    "password_hash": hp("Customer@1234"),
                    "phone":         phone,
                    "address":       address,
        ...
        orders_raw = [
            (0, [(0, 1), (16, 1)], "delivered", 15),
            (0, [(5, 1)],          "confirmed",  14),
            (1, [(2, 1), (17, 1)], "delivered",  13),
            (1, [(10, 1)],         "confirmed",  12),
            (2, [(3, 1), (4, 1)],  "delivered",  11),
            (2, [(13, 1)],         "pending",    10),
            (3, [(6, 1)],          "delivered",   9),
            (3, [(15, 2), (18, 1)],"confirmed",   8),
            (4, [(7, 1), (19, 1)], "delivered",   7),
            (4, [(11, 1)],         "pending",     6),
            (5, [(1, 1), (17, 1)], "delivered",   5),
            (6, [(8, 1)],          "confirmed",   4),
            (7, [(14, 1)],         "delivered",   3),
            (8, [(9, 1), (15, 1)], "delivered",   2),
            (9, [(12, 1)],         "confirmed",   1),
        ]
        orders = []
        oc = oic = 0
        for c_idx, items, status, days_ago in orders_raw:
            ...
            oc += 1
        db.session.flush()
        print(f"  ✔ Orders:      {oc} created, {len(orders) - oc} skipped")
        print(f"  ✔ Order Items: {oic} created")
        ...
        # ── 9. Cart ───────────────────────────────────────────────────────────
        cart_raw = [
            (0,  7, 1), (0, 19, 2), (1,  4, 1), (1, 18, 1),
            (2,  6, 1), (3,  9, 1), (4, 14, 1), (5, 11, 1),
            (6, 17, 1), (7,  0, 1), (8,  5, 1), (9, 13, 1),
        ]
        cart_c = 0
        ...
        recommendations_raw = [
            ...
            (3,  8, 0.9300, "Based on your iPhone purchase, OnePlus 12 is a popular alternative."),
            ...
            (7,  5, 0.9120, "Customers who own HP Spectre often pair it with Galaxy S24."),
        ...
        for c_idx, p_idx, score, explanation in recommendations_raw:
            _, created = get_or_create(
                ...
                defaults={"score": score, "explanation": explanation},
            )
            if created: rec_c += 1
        db.session.flush()
        print(f"  ✔ Recommendations: {rec_c} created")
        ...
        print("✅ Seed complete!")
        print("\n📊 Expected counts:")
        print("   admins: 3 | vendors: 5 | products: 20 | customers: 10")
        print("   orders: 15 | order_items: 20 | payments: 13")
        print("   reviews: 30 | cart: 12 | recommendations: 50")
