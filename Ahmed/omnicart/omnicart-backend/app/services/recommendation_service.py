...
def get_hybrid_recommendations(customer_id: int, product_ids: list,
                                recently_viewed_id: int = None,
                                top_n: int = 5) -> list:
    try:
        svd_scores = dict(get_svd_recommendations(
            str(customer_id), product_ids, top_n=len(product_ids)))
        tfidf_scores = {}
